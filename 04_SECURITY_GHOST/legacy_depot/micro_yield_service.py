...
LOG_FILE_PATH = "/root/MIOS_SOVEREIGN_OS/04_SECURITY_GHOST/legacy_depot/micro_yield.log"
...
TRADES_JSONL_PATH = "/root/MIOS_SOVEREIGN_OS/04_SECURITY_GHOST/legacy_depot/data/nano_yield_trades.jsonl"
ACTIVE_POSITIONS_PATH = "/root/MIOS_SOVEREIGN_OS/04_SECURITY_GHOST/legacy_depot/data/nano_yield_active_positions.json"
...
class NanoYieldAutomationEngine:
    ...
    def __init__(self):
        ...
        self.session_gross_profit_try = 0.0
        self.session_gross_loss_try = 0.0
    ...
    def fetch_live_sol_try_rate(self) -> float:
        """Canlı SOL/TRY kurunu döner (SOL/USD * USD/TRY)."""
        usd_try = self.fetch_live_usd_try_rate()
        sol_usd = 102.0
        if getattr(self, "_cached_prices", None) and "solana" in self._cached_prices:
            p = self._cached_prices["solana"].get("usd", 0.0)
            if p and float(p) > 0:
                sol_usd = float(p)
        return round(sol_usd * usd_try, 2)
    ...
    def execute_nano_trade(
        ...
        with self._state_lock:
            ...
            if self.config.execution_mode == "LIVE":
                ...
                max_slots = getattr(self.config, "max_open_positions", 1)
                if active_count >= max_slots:
                    # FIX v37.4: slot guard throttle 10s
                    _now_sg = time.time()
                    if _now_sg - getattr(self, "_slot_guard_ts", 0.0) >= 10.0:
                        self._slot_guard_ts = _now_sg
                ...
                trade_sol = getattr(self.config, "trade_budget_sol", 0.001)
                buy_floor = getattr(self.config, "dynamic_buy_floor_sol", 0.002)
                usd_try = self.fetch_live_sol_try_rate()
                position_size_try = round(trade_sol * usd_try, 2)
                if sol_balance < buy_floor:
                    # FIX v37.4: throttle 30s
                    _now_kw = time.time()
                    if _now_kw - getattr(self, "_kasa_warn_ts", 0.0) >= 30.0:
                        self._kasa_warn_ts = _now_kw
                ...
                ok_q, verified_quote = self.fetch_jupiter_verified_quote(input_mint, output_mint, trade_lamports, slippage_bps=500)
                ...
                if real_tx:
                    ...
                    wallet_mut = f"-{trade_sol:.4f} SOL (+{symbol})"
    ...
    def close_position_by_token(self, token: str, reason: str = "MANUAL_OR_GUARDIAN_CLOSE", live_data: Optional[Dict[str, Any]] = None) -> List[IndividualTradeRecord]:
        """Belirtilen token için tüm açık pozisyonları derhal kapatır."""
        with self._state_lock:
            closed = []
            if live_data is None:
                _, live_data, _ = self.fetch_live_market_data()
            if not live_data:
                live_data = getattr(self, "_cached_prices", {}) or {}
            to_delete = []
            for pid, pos in list(self._active_positions.items()):
                if pos.token == token or token == "ALL":
                    info = self.config.token_universe.get(pos.token, {})
                    exit_price_try = float(live_data.get(info.get("coingecko_id"), {}).get("try", pos.actual_buy_fill_try) or pos.actual_buy_fill_try)
    ...
    def _execute_position_close(self, pos: ActivePositionRecord, exit_price_try: float, exit_reason: str) -> IndividualTradeRecord:
            ...
            try:
                ...
                if kp:
                    ...
                    if ok_q and sell_quote:
                        ...
                        sol_spent = round(getattr(pos, "position_size_try", 4.8) / usd_try, 6)
                        if sol_spent <= 0:
                            sol_spent = getattr(self.config, "trade_budget_sol", 0.001)
                        real_sol_pnl = sol_received - sol_spent
                        net_pnl_try = round(real_sol_pnl * usd_try, 4)
        ...
        if net_pnl_try > 0:
            self.session_gross_profit_try += net_pnl_try
            self.session_win_count += 1
        else:
            self.session_gross_loss_try += net_pnl_try
    ...
    try:
        while True:
            ...
            if ok and live_data:
                ...
                if len(nano_yield_engine._active_positions) < 3 and not nano_yield_engine.is_frozen:
                    ...
                    for sym in candidate_tokens:
                        ...
                        # Fiyat momentumuna dayalı gerçek güven skoru
                        prev_usd = getattr(self, f"_prev_usd_{sym}", p_usd)
                        mom = (p_usd - prev_usd) / max(prev_usd, 1e-9)
                        setattr(self, f"_prev_usd_{sym}", p_usd)
                        sim_confidence = min(0.95, max(0.55, 0.70 + mom * 50))
            ...
            if now_t - last_heartbeat >= 4.0:
                ...
                usd_try = nano_yield_engine.fetch_live_sol_try_rate()
                sol_try_val = curr_sol * usd_try
            ...
            # v37.4: 60s kasa + kar/zarar ozeti
            if now_t - getattr(nano_yield_engine, "_last_summary_ts", 0.0) >= 60.0:
                nano_yield_engine._last_summary_ts = now_t
                _sol = getattr(nano_yield_engine, "_cached_sol_bal", 0.0)
                _sol_try = nano_yield_engine.fetch_live_sol_try_rate()
                _tc  = nano_yield_engine.session_trade_count
                _wc  = nano_yield_engine.session_win_count
                _lc  = _tc - _wc
                _net = nano_yield_engine.total_realized_pnl_try
                _kar = nano_yield_engine.session_gross_profit_try
                _zar = nano_yield_engine.session_gross_loss_try
                _wr  = round((_wc / _tc * 100) if _tc > 0 else 0.0, 1)
                logger.info(
                    "\n" + "=" * 56 + "\n"
                    + f"  [KASA & KAR/ZARAR OZETI]\n"
                    + f"  Kasa    : {_sol:.6f} SOL  (~{_sol*_sol_try:.2f} TL)\n"
                    + f"  Net PnL : {_net:+.2f} TL  |  Kar: +{_kar:.2f} TL  |  Zarar: {_zar:.2f} TL\n"
                    + f"  Islemler: {_tc} toplam | {_wc} kazanc | {_lc} kayip | %{_wr} basari\n"
                    + "=" * 56
                )
