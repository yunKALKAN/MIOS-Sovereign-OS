...
class NanoYieldConfig:
    ...
    target_yield_per_sec_try: float = 0.05
    min_position_size_try: float = 60.00
    max_position_size_try: float = 300.00
    default_micro_budget_try: float = 75.00

    # Risk & Kâr / Zarar Kapıları
    ai_confidence_gate: float = 0.70         # %90+ Ultra Yüksek Güven Filtresi
    max_slippage_bps: int = 25               # %0.25 Maksimum Kayma Toleransı
    take_profit_bps: int = 120               # %1.20 Take Profit Hedefi
    hard_stop_bps: int = 250                 # %2.50 Stop-Loss Sermaye Koruma (Genişletilmiş Tolerans)
    # Staged Take-Profit (Kademeli Erken Kâr Al)
    staged_tp_enabled: bool = True           # Kademeli Kâr Realizasyonu Aktif
    staged_tp_seconds: int = 90              # 90. saniyeden sonra devreye girer
    staged_tp_bps: int = 50                  # +%0.40 kâr (40 bps) görüldüğünde realize et
    min_liquidity_usd: float = 50000.0       # Minimum Havuz Likiditesi ($50k)
    max_price_age_sec: float = 5.0           # Maksimum Fiyat Yaşı (5 saniye)
    max_hold_time_ms: int = 180000           # Maksimum Pozisyonda Kalma Süresi (180s / 3dk)
    ...
    max_daily_loss_try: float = 10000.00     # Günlük Maksimum Zarar Limiti (Esnek)
    max_consecutive_loss: int = 50           # Maksimum Ardışık Kayıp Sayısı

    # ============================================================
    # AUTONOMOUS CAPITAL PROTECTION v1
    # ============================================================
    trade_budget_sol: float = 0.001
    ata_rent_reserve_sol: float = 0.0005
    safety_buffer_sol: float = 0.0005
    ...
    active_positions_path: str = "/root/MIOS_SOVEREIGN_OS/04_SECURITY_GHOST/legacy_depot/data/nano_yield_active_positions.json"
    ata_rent_sol: float = 0.00203928         # Standart SPL ATA rent maliyeti (~0.00204 SOL)
    sol_reserve_sol: float = 0.0005          # Cüzdanda zorunlu tutulması gereken SOL rezervi
    guardian_heartbeat_timeout_sec: float = 30.0 # Guardian'dan haber alınamazsa yeni işlem engeli
    signal_dedup_window_sec: float = 10.0    # Aynı sinyalin tekrar icrasını önleme penceresi (10s)
    ...
    def __post_init__(self):
        self.dynamic_buy_floor_sol = 0.002
